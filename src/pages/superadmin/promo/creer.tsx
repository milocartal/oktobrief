...
const CreerPromo: NextPage<InferGetServerSidePropsType<typeof getServerSideProps>> = () => {
  ...
  return (
    <>
      ...
      <main className="flex h-screen min-w-screen flex-col items-start justify-start bg-[#F3F3F3] pl-[100px] pt-[40px] gap-3">
            ...
            <fieldset className="bg-white h-full w-[59%] p-5 flex flex-col gap-3 rounded-lg">
              ...
              <fieldset>
                ...
                <QuillNoSSRWrapper
                  ...
                  className="pb-11 bg-white w-full h-[180px]"
                  modules={modules}
                  onChange={() => setDesc}
                />
              </fieldset>
              ...
              <fieldset className='overflow-y-auto'>
                ...
                <div className='grid grid-cols-2 gap-3 overflow-y-auto'>
                  {ref && ref.map((item) => {
                    return (
                      <div className={`py-5 flex flex-row items-center justify-center shadow-[inset_3px_4px_12px_0px_rgba(0,0,0,0.25)] text-center text-sm rounded-lg ${item.id === selected?.id ? 'bg-[#0E6073] text-white' : 'bg-white'} hover:cursor-pointer`} onClick={() => setSelected(item)} key={item.id}>
                        {item.title}
                      </div>
                    )
                  })}
