    ...
    return (
        <div className="fixed top-0 left-0 w-[100px] bg-[#0e6073] h-screen flex flex-col items-center text-white text-sm justify-between pt-5 pb-8 px-2">
            ...
            <div className="flex flex-col items-center justify-start gap-[3px]">
                ...
                {sessionData?.user.superadmin && <Link href={"/superadmin"} className="flex flex-col items-center justify-center transitionn rounded-xl hover:bg-[#2EA3A5] w-full py-3 text-center"><Image width={50} height={50} src="/superhero.svg" className="w-7 mb-1" alt="superhro icon"/>Super Admin</Link>}
            </div>
            ...
            {open &&
                <div className="bg-white absolute left-28 bottom-8 px-5 py-4 w-72">
                    <span className="flex flex-row justify-start items-center">
                        {sessionData?.user.image && <Image width={50} height={50} loader={() => userPFP} src={userPFP} className="w-12 h-12 rounded-full object-cover mr-3" alt="Photo de profil utilisateur" />}
                        <div>
